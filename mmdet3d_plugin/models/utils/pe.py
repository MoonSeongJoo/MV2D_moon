...
def pos2posemb3d(pos, num_pos_feats=128, temperature=10000):
    ...
    def safe_stack(tensor):
        ...
        if length % 2 != 0:
            tensor = tensor[..., :-1]
        return torch.stack((tensor[..., 0::2].sin(), tensor[..., 1::2].cos()), dim=-1).flatten(-2)

    pos_x_emb = safe_stack(pos_x)
    pos_y_emb = safe_stack(pos_y)
    pos_z_emb = safe_stack(pos_z)

    posemb = torch.cat((pos_y_emb, pos_x_emb, pos_z_emb), dim=-1)
    return posemb
