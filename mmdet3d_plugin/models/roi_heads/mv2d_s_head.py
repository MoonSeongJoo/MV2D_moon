...
class MV2DSHead(MV2DHead):
    ...
    def center_pts_to_bev(self,center_pts, bev_shape, x_range, y_range):
        """
        center_pts: tensor of shape [N,3] (x,y,z)
        bev_shape: tuple (H, W) BEV grid size, e.g. (225,400)
        x_range: tuple (x_min, x_max) BEV 영역 x 범위
        y_range: tuple (y_min, y_max) BEV 영역 y 범위
        
        Returns:
        bev_coords: tensor of shape [N,2], (x_idx, y_idx) in BEV grid coordinates
        """

        # Extract x,y coordinates
        x_points = center_pts[:, 0]
        y_points = center_pts[:, 1]

        # Clip points to range limits
        x_points = torch.clamp(x_points, x_range[0], x_range[1])
        y_points = torch.clamp(y_points, y_range[0], y_range[1])

        # Normalize points to [0, 1]
        x_norm = (x_points - x_range[0]) / (x_range[1] - x_range[0])
        y_norm = (y_points - y_range[0]) / (y_range[1] - y_range[0])

        # Scale normalized coordinates to BEV grid size
        # In image coordinates, y axis is usually top to bottom increasing, so invert y axis if needed
        bev_x = x_norm * (bev_shape[1] - 1)
        bev_y = (1 - y_norm) * (bev_shape[0] - 1)  # y-axis invert for image coordinates

        bev_coords = torch.stack([bev_x, bev_y], dim=1)  # shape [N,2], float for interpolation or int for indexing
    ...
    # def _bbox_forward_denoise(self, x, proposal_list, img_metas): # for original 
        ...
        bev_shape = (225, 400)
        query_bev = self.center_pts_to_bev(detection_xyz, bev_shape=bev_shape,
                               x_range=(self.pc_range[0], self.pc_range[3]), 
                               y_range=(self.pc_range[1], self.pc_range[4]))
        query_bev_normalized = query_bev / torch.tensor([bev_shape[1]-1, bev_shape[0]-1], 
                                                          device=query_bev.device)
