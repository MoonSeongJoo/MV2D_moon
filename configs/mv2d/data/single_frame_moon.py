...
data_root = '/workspace/MV2D_moon/data/nuscenes/'
...
train_pipeline = [
    ...
    # dict(type='ResizeCropFlipImageMono', data_aug_conf=ida_aug_conf, with_bbox_2d=True,resize_only=False, training=True),
    dict(type='PointToMultiViewDepth',  grid_config=grid_config,downsample=1),
    dict(type='gt_bbox_trasform'),
    ...
    dict(type='PadMultiViewImage', size_divisor=32), # size_divisor=32 (original)
    ...
    dict(type='CollectMono3D',
         ...
         keys=['gt_bboxes_3d', 'gt_labels_3d', 'gt_bboxes_2d', 'gt_labels_2d', 'gt_bboxes_2d_to_3d', 'gt_bboxes_ignore',
               'img','lidar_depth_mis','lidar_depth_gt','mis_KT','mis_Rt','gt_KT','gt_KT_3by4','raw_points']),
]
test_pipeline = [
    dict(type='LoadPointsFromFile',coord_type='LIDAR',load_dim=5,use_dim=[0,1,2,3,4],file_client_args=file_client_args),
    dict(type='LoadPointsFromMultiSweeps',sweeps_num=10,use_dim=[0, 1, 2, 3, 4],),
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='CollectMono3D', debug=False,
                 keys=['img','lidar_depth_mis','lidar_depth_gt','mis_KT','mis_Rt','gt_KT','gt_KT_3by4','raw_points'])
        ])
]
